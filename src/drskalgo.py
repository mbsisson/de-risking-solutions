...
#         # Get impact, i.e. \phi^t_L
#         IMP = solutionvectordict.get('IMP', 0.0)
#         alldata['algo']['iterations'][iteration]['IMP'] = IMP

#         # Compute the max impact, i.e. z^t and \phi^t_\max
#         maximpact = drsk_getimpact_softmax(alldata, solutionvectordict)
#         alldata['algo']['iterations'][iteration]['maximpact'] = maximpact
#         breakexit('computed impact')

#         # Log objective and impact values
#         log.joint('Iteration %d.  Obj %.5g  Cost %.5g  IMP %.5g  Max-Impact %.5g.\n' %(iteration, objval, cost, IMP, maximpact))
#         if iteration > 0:
#             c_0 = alldata['algo']['iterations'][0]['cost']
#             maximpact_0 = alldata['algo']['iterations'][0]['maximpact']
#             log.joint('  > %.4f increase over original cost %.2f\n'%(cost / c_0, c_0))
#             log.joint('  > %.4f decrease from original maximpact %.2f\n'%(maximpact / maximpact_0, maximpact_0))

#         if iteration == 0: 
#         # Add impact variable, IMP, if first iteration (need first impact value to tune weights) TODO: move before loop somehow
#             retcode, condition = drsk_add_impactvar(alldata)
#             if retcode:
#                 log.joint('add_impactvar returns %d at iter %d.\n' %(retcode, iteration))
#                 return retcode, condition
...
#         # Convergence check
#         if (maximpact - IMP) / maximpact < tol:
#             log.joint('  DONE: impact converged\n')
#             break
#         else:
#             log.joint('  NOTDONE: keep running.\n')
...
#         objval = float(alldata['algo']['iterations'][i]['objval'])
#         cost = float(alldata['algo']['iterations'][i]['cost'])
#         IMP = float(alldata['algo']['iterations'][i]['IMP'])
#         maximpact = float(alldata['algo']['iterations'][i]['maximpact'])
#         log.joint('  Iteration %d   Objval %10.10f   Cost %10.10f   IMP %10.10f   Max-Impact %10.10f\n'%(i, objval, cost, IMP, maximpact))
#     log.joint('\n')

#     c_0 = alldata['algo']['iterations'][0]['cost']
#     imp_0 = alldata['algo']['iterations'][0]['maximpact']
#     c_last = alldata['algo']['iterations'][iteration]['cost']
#     imp_last = alldata['algo']['iterations'][iteration]['maximpact']
#     log.joint('Impact Decrease (lambda): %g\n'%(imp_last / imp_0))
#     log.joint('Cost Increase (multiplier): %g\n'%(c_last / c_0))
